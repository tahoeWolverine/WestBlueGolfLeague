...
<HTML>
<HEAD>
	<TITLE>West Blue Golf League - 1998 Standings</TITLE>
</HEAD>

<BODY bgcolor="white"> 
<center><h2><font color="Green">1998 Standings</font></h2></center>
<%
Set Conn = Server.CreateObject("ADODB.Connection")
Conn.Open "wbgdb"
Set RSA = Conn.Execute("SELECT Team FROM TeamList WHERE Division='A'")
Set RSB = Conn.Execute("SELECT Team FROM TeamList WHERE Division='B'")
j = 0
Do While Not RSA.EOF
	...
	TeamAArray (j) = RSA("Team")
	Set RS1 = Conn.Execute("SELECT Points1 FROM ResultsList WHERE Team1='" & TeamAArray(j) & "'")	
	Do While Not RS1.EOF
		TotalPointsAArray(j) = TotalPointsAArray(j) + RS1("Points1")
		TotalScores = TotalScores + 1
		RS1.MoveNext
	Loop %>
<% 	Set RS2 = Conn.Execute("SELECT Points2 FROM ResultsList WHERE Team2='" & TeamAArray(j) & "'")	
	Do While Not RS2.EOF
		TotalPointsAArray(j) = TotalPointsAArray(j) + RS2("Points2")
		TotalScores = TotalScores + 1
		RS2.MoveNext
	Loop
	Set RS = Conn.Execute("SELECT Player FROM PlayerList WHERE Team='" & TeamAArray(j) & "'")
	Do While Not RS.EOF
...
Do While Not RSB.EOF
	...
	TeamBArray (j) = RSB("Team")
	Set RS1 = Conn.Execute("SELECT Points1 FROM ResultsList WHERE Team1='" & TeamBArray(j) & "'")	
	Do While Not RS1.EOF
		TotalPointsBArray(j) = TotalPointsBArray(j) + RS1("Points1")
		TotalScores = TotalScores + 1
		RS1.MoveNext
	Loop %>
<% 	Set RS2 = Conn.Execute("SELECT Points2 FROM ResultsList WHERE Team2='" & TeamBArray(j) & "'")	
	Do While Not RS2.EOF
		TotalPointsBArray(j) = TotalPointsBArray(j) + RS2("Points2")
		TotalScores = TotalScores + 1
		RS2.MoveNext
	Loop
	Set RS = Conn.Execute("SELECT Player FROM PlayerList WHERE Team='" & TeamBArray(j) & "'")
	Do While Not RS.EOF
		PlayerCount = PlayerCount + 1
		RS.MoveNext
	Loop %>
    ...
    <td>		
		<table>
			...
			<% For z = UBound(TeamAArray) to 0 step - 1 %>
				...
				<tr>
			    <td valign="top">
					<b><font color="Green"><%= Team %></font></b>
				</td>
				<td align="center">
