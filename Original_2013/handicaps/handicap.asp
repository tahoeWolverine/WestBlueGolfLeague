...
%>
<% 
' Used starting in 2011 when we changed the handicap formula to carry over four scores from the previous year.
...
RS.Close
x = 0
' For OLD players only, fill in any missing scores with week0scores. This is the account for the carry over of 4 scores from the previous year. 
if Status = "old" then
	For x = (UBound(WeekArray) + 1) to (iScoresToEval - 1) 
		ReDim Preserve WeekArray (x) 
		ReDim Preserve ScoreArray (x) 
		ReDim Preserve PointsArray(x) 		
		WeekArray(x) = 0 
...
%>
<% 
' This is different than the orignal PopulateArrays in that in retrievs the week0score from the Player Table.
' Prior to this, a players week0score was embeded within the results table.
...
%>
<% 
' This is used for years prior to 2009 when the week0score was embeded with the Results table
' Also, week0score was weighted the same as any other weekly score...that later changed.
