...
- (void)calculateTeamBoardWithName:(NSString *)name
	...
	if (teamCount > 0) {
		[WBBoardData createBoardDataForEntity:[WBPeopleEntity leagueAverageInContext:moc] leaderBoard:board value:(totalLeagueValue / teamCount) rank:0 year:year];
	}
...
- (void)calculatePlayerBoardWithName:(NSString *)name
	...
	// Create league average for board
	if (playerCount > 0) {
		[WBBoardData createBoardDataForEntity:[WBPeopleEntity leagueAverageInContext:moc] leaderBoard:board value:(totalLeagueValue / playerCount) rank:0 year:year];
	}
